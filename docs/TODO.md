...
> Priorities: **P0** = pipeline-blocking / immediate fix, **P1** = important for thesis quality, **P2** = nice-to-have.

---

## P0 — Immediate (Pipeline & Thesis Blocking)

These items directly affect experiment validity, thesis results, or reproducibility. Nothing in the next section is worth starting until these are complete.
...
### Experiments Not Yet Started (Blocking Thesis)
...
### Thesis Result Tables (Blocking Write-up)

| # | Task | Depends On | Notes |
|---|------|------------|-------|
| 1.8 | Fill thesis result tables (accuracy, F1, AUC per dataset) | 1.1–1.7 | `docs/thesis.md` tables at §4.3 have pending results. |
...
## P1 — Important (Strengthen Thesis Quality)
...
| # | Task | Notes |
|---|------|-------|
| 3.1 | Ablation: vary number of FL clients (2, 4, 8) | Communication cost vs accuracy trade-off. Adds depth to §4.5 but not required. |
...
| `docs/thesis.md` had `[PLACEHOLDER]` text in abstract, comparison table said "small" | Fixed: de-placeholdered abstract, table updated to "paper variant". |
...
| 5-fold CV | Yes | Single 85/15 split | Not implemented — documented as limitation in thesis §3.4 |
